...
# names(mylove) = c("mtor","cancer")
 ...
 mylove = data.frame(design$pi3k,design$cancer)
 names(mylove) = c("pi3k","cancer")
...
if (F) {
  ...
  save( mydata_mtor,mydata_mtor_cancer,file = "合并模块后pi3k_akt_mtor_turquoise模块核心基因初步结果.Rdata")
...
if (F) {
 ...
 merge_pi3k_akt <- data.frame(moduleGenes=colnames(datExpr)[moduleGenes],MM,MMP,GS,GSP)
  ## cancer
  ...
  merge_pi3k.akt_cancer <- data.frame(moduleGenes=colnames(datExpr)[moduleGenes],MM,MMP,GS,GSP)
  
  
  setwd("C:/Users/Cloudy/Desktop/论文写作/文章写作预分析/dat_output")
  
  save(  merge_pi3k_akt,   merge_pi3k.akt_cancer,file = "pi3k_akt.tan模块核心基因初步结果.Rdata")
