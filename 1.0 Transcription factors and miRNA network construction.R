...
miRNA_taball %>% 
  # distinct(miRNA,gene,.keep_all = T) %>%   #保留，不要去重
...
# 写出 ----------------------------------------------------------------------
write.xlsx(miRNA_taball_unique,"dat_output/miRNA network data.xlsx")

write.xlsx(TF_tab,"dat_output/TF Network data.xlsx")
