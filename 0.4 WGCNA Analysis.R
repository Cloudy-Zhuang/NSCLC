...
save(datExpr,pheno_mtor,pheno_pi3k,file = "WGCNA准备数据.Rdata")

## WGCNA ANALYSIS
load("dat_output/WGCNA准备数据.Rdata")
library(WGCNA)
...
if (F) {
  ...
  save( mydata_mtor,mydata_mtor_cancer,file = "pi3k_akt_mtor蓝色模块核心基因初步结果.Rdata")
...
## PI3K-AKT
 if (F) {
    ...
    mydata_pi3k_akt <- data.frame(moduleGenes=colnames(datExpr)[moduleGenes],MM,MMP,GS,GSP)
    ## cancer
    ...
    mydata_pi3k.akt_cancer <- data.frame(moduleGenes=colnames(datExpr)[moduleGenes],MM,MMP,GS,GSP)
  
    
    setwd("C:/Users/Cloudy/Desktop/论文写作/文章写作预分析/dat_output")
    
    save( mydata_pi3k_akt, mydata_pi3k.akt_cancer,file = "pi3k_akt鲱红色模块核心基因初步结果.Rdata")
